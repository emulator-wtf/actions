...
export async function authenticateOidc(
...
): Promise<OidcAuthenticateResponse> {
  const client = new HttpClient();

  const response = await client.postJson<
    OidcAuthenticateResponse | ErrorResponse
  >("https://api.emulator.wtf/auth/oidc", request);
  ...
  if (response.statusCode >= 200 && response.statusCode < 300) {
    ...
    if (isErrorResponse(response.result)) {
      throw new Error(
        `Unexpected API response shape: received error body for successful (2xx) status: ${response.result.message}`,
      );
    }
  ...
  throw Error(
    `API call to emulator.wtf failed with status code ${response.statusCode}`,
  );
}
